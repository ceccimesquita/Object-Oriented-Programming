    ...
    int getBalance() {
      int op = 0;
        for(auto elem : operations){
          if(elem->getLabel() == GIVE || elem->getLabel() == PLUS){
            op += elem->getValue();
          }else{
            op -= elem->getValue();
          }
...
class Agiota {
    ...
    void pushOperation(Client& cli, Label label, int value) {
        shared_ptr<Operation> aux = make_shared<Operation>(nextOpId, cli.getName(), label, value);
        this->nextOpId++;
        alive_oper.push_back(aux);
        cli.addOperation(aux);
    }
...
public:
    ...
    Client* getClient(std::string name) {
        if(searchClient(name) == -1){
          write("fail: cliente nao existe");
          return nullptr;
        }else{
          Client* aux = nullptr;
          aux = &alive_list.at(searchClient(name));
          return aux;
        }
    }
    ...
    void addClient(std::string name, int limite) {
        if(searchClient(name) == -1){
          Client aux(name, limite);
          alive_list.push_back(aux);

        }else{
          write("fail: cliente ja existe");
        }
    ...
    void give(std::string name, int value) {
        if(getClient(name) != nullptr){
          ...
          aux = getClient(name);
          aux1 = *aux;
          if(aux1.getBalance()+value <= aux1.getLimite()){
            pushOperation(*aux, GIVE, value);
          }else{
            write("fail: limite excedido");
          }
        }
    }
    ...
    void plus() {
       for(auto& elem : alive_list){
        int value = (elem.getBalance() + 9) / 10; // Arredonda para cima
        pushOperation(elem, PLUS, value);
      }
      
      std::vector<std::string> clientsToKill;
      
      for(auto& elem : alive_list){
        if(elem.getBalance() > elem.getLimite()){
          clientsToKill.push_back(elem.getName());
        }
      }
      
      for(auto& name : clientsToKill){
        kill(name);
      }
    }
    ...
    void kill(std::string name) {
        ...
        if(searchClient(name) != -1){
          Client aux = alive_list.at(searchClient(name));
          alive_list.erase(alive_list.begin() + searchClient(name));
          death_list.push_back(aux);

          for(auto elem : alive_oper){
            if(elem->getName() == name)
              death_oper.push_back(elem);
          }
